...
from monai.networks.layers.factories import Conv, LayerFactory
...
class SimpleUNet(nn.Module):
    def __init__(
        ...
        self.down_conv1 = nn.Sequential(
            nn.utils.spectral_norm(conv_mod(inc1, c, 3, 1, 1)),
            get_act_layer(act),
        ...
        self.down_conv2 = nn.Sequential(
            nn.utils.spectral_norm(conv_mod(inc2, 2*c, 3, 1, 1)),
            get_act_layer(act),
        ...
        self.down_conv3 = nn.Sequential(
            nn.utils.spectral_norm(conv_mod(inc3, 4*c, 3, 1, 1)),
            get_act_layer(act),
        ...
        self.down_conv4 = nn.Sequential(
            nn.utils.spectral_norm(conv_mod(inc4, 8*c, 3, 1, 1)),
            get_act_layer(act),
        ...
        # self.out_conv = conv_mod(c, out_channels, 3, 1, 1)
        ...
        elif downsample_mode == 'nearest':
            self.downsample = [lambda x: F.interpolate(x, scale_factor = 1/2, mode = 'nearest'), lambda x: F.interpolate(x, scale_factor = 1/2, mode = 'nearest'), lambda x: F.interpolate(x, scale_factor = 1/2, mode = 'nearest')]
        if upsample_mode == 'pixel_shuffle':
            ...
            self.upsample = [lambda x: F.interpolate(x, scale_factor = 2, mode = 'nearest'), lambda x: F.interpolate(x, scale_factor = 2, mode = 'nearest'), lambda x: F.interpolate(x, scale_factor = 2, mode = 'nearest')]
            if last_pixelshuffle:
