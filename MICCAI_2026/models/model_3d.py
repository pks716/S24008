...
def Normalize(in_channels):
    """GroupNorm with automatic num_groups for divisibility"""
    # Find best num_groups (try 32, 16, 8, 4, 2, 1)
...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        if self.with_conv:
            pad = (0, 1, 0, 1, 0, 1)  # depth, height, width
...
class Model(nn.Module):
    ...
    def forward(self, x, t):
        # For 3D: x.shape should be [batch, channels, depth, height, width]
        # Assuming cubic volumes, but you can modify for non-cubic
        # assert x.shape[2] == self.resolution and x.shape[3] == self.resolution and x.shape[4] == self.resolution
